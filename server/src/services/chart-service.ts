...
import { DailyData, DayInfo, FromToInfo } from '../customType/chart.type';
...
class ChartService {
  ...
  async getDailyChart(dayInfo: DayInfo): Promise<DailyData | null> {
    //받은 날짜로 계산
    ...
    for (let i = 0; i < data.length; i++) {
      dailyData.todayWeight.push(data[i].todayWeight);
